      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 flex flex-col items-center text-center">
            ...
            <span className={`mt-3 px-3 py-1 rounded-full text-xs font-medium ${user?.role && roleBadge[user.role] ? roleBadge[user.role] : roleBadge.STUDENT}`}>
              {user?.role}
            </span>
          </div>

          {/* Account stats */}
          <SectionCard title="Account Info" icon={<Shield className="w-5 h-5 text-green-400" />}>
          ...
          <SectionCard title="Change Password" icon={<Lock className="w-5 h-5 text-green-400" />}>
            <form onSubmit={handlePasswordSave} className="space-y-4">
              ...
              {[
                ...
                const k = key as 'current' | 'new' | 'confirm';
                const f = field as keyof typeof pwForm;
                return (
                  <Field key={key} label={label}>
                    <div className="relative">
                      <input
                        ...
                        value={pwForm[f]}
                        onChange={(e) =>
                          setPwForm((p) => ({ ...p, [f]: e.target.value }))
                        }
                        required
                      />
                      <button
