 ...
 * Provides contextual hints based on lab type
 */
...
/**
 * Get mock response based on lab category
 */
...
export const chat = asyncHandler(async (req, res) => {
  const { message, sessionId, labId, conversationHistory = [] } = req.body;

  if (!message || message.trim().length === 0) {
    throw new ApiError("Message is required", 400);
  }

  // Get lab context for personalized responses
...
/**
 * @desc    Get AI suggestions for current lab
 * @route   GET /api/chat/suggestions
 * @access  Private
 */
export const getSuggestions = asyncHandler(async (req, res) => {
  const { sessionId, labId } = req.query;
